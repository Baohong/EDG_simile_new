...
Exhibit.OLMapView._settingSpecs = {
    ...
    "numZoomLevels":    { type:"int",       defaultValue: 18},
    ...
    "type":             { type: "enum",     defaultValue: "vmap", choices: [ "osm", "wms", "gmap", "gsat", "ghyb", "gter", "vmap", "vsat", "vhyb", "ymap", "ysat", "yhyb" ] },
...
Exhibit.OLMapView.prototype._constructMap = function(mapDiv) {
	...
	var availableLayers = [osm];
	var availability = { "osm" : osm};
*/
	var availableLayers = [];
	var availability = { };
	...
	var apiKey = "AkpioNuOZOdJiXnWUQtWloLIACB-AJpwTpvS884VsCzcmjWtT9Tf3l3qvREekSgF";
        ...
        var vmap = new OpenLayers.Layer.Bing({
                key: apiKey,
		numZoomLevels: settings.numZoomLevels,
                type: "Road"
        });
	    availableLayers.push(vmap);
	    availability["vmap"] = vmap;
	...
	if (typeof availability[settings.type] != "undefined") {
	    //map.setBaseLayer(availability[settings.type]);
	}
	...
	availableLayers = null;
        ...
        //AE map.addControl(new OpenLayers.Control.PanPanel());
...
Exhibit.OLMapView.prototype._reconstruct = function() {
    this._clearOverlays();
    //AE this._dom.legendWidget.clear();
    this._itemIDToMarker = {};
...
Exhibit.OLMapView.prototype._rePlotItems = function(unplottableItems) {
    ...
    if (hasColorKey) {
        ...
        if (colorCoder._gradientPoints != null) {
            ...
            //AE legendGradientWidget.addGradient(this._colorCoder._gradientPoints);
        } else {
        ...
        if (colorCodingFlags.others) {
            //AE legendWidget.addEntry(colorCoder.getOthersColor(), colorCoder.getOthersLabel());
        }
        if (colorCodingFlags.mixed) {
            //AE legendWidget.addEntry(colorCoder.getMixedColor(), colorCoder.getMixedLabel());
        }
        if (colorCodingFlags.missing) {
            //AE legendWidget.addEntry(colorCoder.getMissingColor(), colorCoder.getMissingLabel());
        }
    }
    ...
    if (hasSizeKey) {
        ...
        if (sizeCoder._gradientPoints != null) {
            ...
            for (var k = 0; k < keys.length; k++) {
                ...
                //AE legendWidget.addEntry(size, key, 'size');
            }
           } else {       
            for (var k = 0; k < keys.length; k++) {
                var key = keys[k];
