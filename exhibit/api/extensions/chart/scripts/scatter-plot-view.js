    ...
    if (currentSize > 0) {
        ...
        currentSet.visit(function(itemID) {
            ...
            self._getXY(itemID, database, function(xy) {
                if ("x" in xy && "y" in xy) xys.push(xy);
            });
        ...
        canvasDiv.style.height = (this._settings.plotHeight-40)+"px";
        canvasDiv.style.width = this._settings.plotWidth+"px";
        ...
        var makeMakeLabel = function(interval, unscale) {
            ...
            if (interval >= 1000000) {
                return function (n) {
                    return Math.floor(unscale(n) / 1000000) + "M";
                };
            } else if (interval >= 1000) {
                return function (n) {
                    return Math.floor(unscale(n) / 1000) + "K";
                };
            } else {
                return function (n) {
                    return unscale(n);
        ...
        var makeMakeLabelAE = function(interval, unscale) {
            return function (n) {
                var d = new Date(2008,10,1+n);
                return d.getFullYear() + "-" + (d.getMonth()+1) + "-" + d.getDate();
            };
        };
        //AE var makeLabelX = makeMakeLabel(xInterval, unscaleX);
        var makeLabelX = makeMakeLabelAE(xInterval, unscaleX);
        ...
        var addPointAtLocation = function(xyData) {
            ...
            var marker = Exhibit.ScatterPlotView._makePoint(
                ...
                // AE: Quick and dirty hack to modify hover note
                xy.y + " download(s) on " + xyData.items + "."
                );
...
Exhibit.ScatterPlotView._makePoint = function(color, left, bottom, tooltip) {
    ...
    "; width: 2px; height: " + (bottom + 3) + "px; left: " + (left + 3) + "px; bottom: 0px;' title='" + tooltip + "'></div>";
    //AE "; width: 6px; height: 6px; left: " + (left - 3) + "px; bottom: " + (bottom + 3) + "px;' title='" + tooltip + "'></div>";
